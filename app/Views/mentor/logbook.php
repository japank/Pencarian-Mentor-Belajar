            ...
            <div class="scroll-sidebar">
                ...
                <nav class="sidebar-nav">
                    <ul id="sidebarnav">
                        ...
                        <li class="sidebar-item"> <a class="sidebar-link waves-effect waves-dark sidebar-link active" style="background-color:#1a9bfc;color:white" href="<?= base_url(); ?>/mentor/request-accepted" aria-expanded="false"><i class="mdi mdi-book-multiple" style="color:white"></i><span class="hide-menu">Mentoring</span></a></li>
            ...
            <div class="container-fluid">
                ...
                <div class="row">
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-body">
                                <h4 class="card-title" style="position: absolute; right: 7%;">
                                    <?php
                                    if ($requestDetail->status_mentoring == '0') {
                                        echo '<span class="badge bg-warning">Proses Mentoring</span>';
                                    } else {
                                        echo '<span class="badge bg-success">Mentoring Selesai</span>';
                                    }
                                    ?></h4>
                                <h4 class="card-title">Logbook <?= $requestDetail->username_siswa ?></h4>
                                <h6 class="card-subtitle"> <?= $requestDetail->topic ?> <?= $requestDetail->description ?></h6>
                            </div>
                            <?php
                            if ($requestDetail->status_mentoring == '0') { ?>
                                <button type="button" class="btn btn-primary btn-sm tomboltambah" style="width:fit-content;margin-left:3%"><i class="fa fa-plus-circle"></i> Tambah Data</button><br><br>
                            <?php } ?>
                ...
                <?php
                if ($requestDetail->status_mentoring == '0') { ?>
                    <button type="button" class="btn btn-success btn-sm " style="width:fit-content;position: absolute; right: 7%;" onclick=" requestDone('<?= $requestDetail->username_siswa ?>','<?= $requestDetail->id_request_mentor ?>')"><i class="fa fa-question"></i> Tandai sebagai mentoring selesai </button>
...
<script type="text/javascript">
    function studentLogbook() {
        $.ajax({
            url: "<?= site_url('logbook/loadDetail/' . $id_request) ?>",
            dataType: "json",
    ...
    function requestDone(username, id_request) {
        Swal.fire({
            title: 'Selesaikan Mentoring',
            text: `Selesaikan Mentoring dengan Siswa "${username}" ?`,
            icon: 'question',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: "post",
                    url: "<?= site_url('mentor/requestDone') ?>",
                    data: {
            ...
            window.location = '<?= base_url(); ?>/mentor/request-accepted';
        })
    }
    $(document).ready(function() {
        ...
        $('.tomboltambah').click(function(e) {
            e.preventDefault();
            $.ajax({
                url: "<?= base_url('logbook/addlogbook/' . $id_request) ?>",
                dataType: "json",
                success: function(response) {
